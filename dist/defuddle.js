...
class Defuddle {
    ...
    parseInternal(overrideOptions = {}) {
        ...
        try {
            ...
            const removalRoot = (mainContent.tagName.toLowerCase() === 'body')
                ? clone
                : mainContent;
            ...
            if (options.removeLowScoring) {
                scoring_1.ContentScorer.scoreAndRemove(removalRoot, this.debug, debugRemovals, mainContent);
            }
    ...
    isHiddenUtilityClass(token) {
        ...
        for (const variant of variants) {
            ...
            if (constants_1.CONDITIONAL_VISIBLE_VARIANTS.has(base)) {
                return false;
            }
            if (constants_1.CONDITIONAL_VISIBLE_VARIANT_PREFIXES.some((prefix) => base.startsWith(prefix))) {
                return false;
            }
        }
        return true;
    ...
    removeBySelector(doc, removeExact = true, removePartial = true, mainContent, debugRemovals) {
        ...
        if (removePartial) {
            ...
            const partialRegex = constants_1.PARTIAL_SELECTORS_REGEX;
